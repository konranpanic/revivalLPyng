...
const steps = [
  {
    ...
    icon: Package,
    title: "カートに入れて決済",
    description: "「購入する」ボタンを押して画面の指示に従うだけ。数分で完了します。",
  },
  {
    number: "2",
    icon: Search,
    title: "スタッフが最終チェック＆梱包",
    description: "発送前にもう一度状態を確認。湿気や衝撃から守る丁寧な梱包で発送します。",
  },
  {
    number: "3",
    icon: Truck,
    title: "追跡番号付きで最速発送",
    description: "発送完了と同時に追跡番号をメールでお知らせ。リアルタイムで確認できます。",
  },
]
...
export function CtaSection() {
  ...
  return (
    <section id="cta-section" ref={sectionRef} className="relative overflow-hidden bg-gray-50 py-24 md:py-32">
        ...
        <div className="mb-16 grid gap-4 md:grid-cols-3">
          {steps.map((step, index) => (
            <div key={index} className="animate-item opacity-0">
              <div className="group relative h-full rounded-2xl bg-white p-8 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-md">
        ...
        <div className="animate-item opacity-0">
          <div className="relative mx-auto max-w-3xl overflow-hidden rounded-3xl bg-black p-10 text-center md:p-16">
            ...
            <p className="mb-8 text-sm text-gray-400">
              同じコンディションの次回入荷は完全未定。気になったなら今が買い時。
            </p>
            ...
            <p className="text-xs text-gray-600">
              ※先着順です。カート中でも他の方が決済を完了すると売切れになります。
            </p>
          </div>
        </div>
      </div>
